...
class LiveTrace:
    ...
    def __init__(self, enabled: bool = False) -> None:
        self.enabled, self._text_open, self._printed = enabled, False, set()
    ...
    async def event_stream_handler(self, _ctx: Any, events: AsyncIterable[Any]) -> None:
        async for event in events:
            if isinstance(event, FunctionToolCallEvent):
                ...
                self._start(event.index, event.part.content)
            elif isinstance(event, PartDeltaEvent) and isinstance(event.delta, TextPartDelta):
                self._write(event.index, event.delta.content_delta)
            elif isinstance(event, PartEndEvent) and isinstance(event.part, TextPart):
                if event.index not in self._printed:
                    self._start(event.index, event.part.content)
                self._close()

    def _start(self, index: int, content: str) -> None:
        if self.enabled:
    ...
    def _text(self, value: Any) -> str:
        return self._redact(
            ...
            else json.dumps(self._safe(value), indent=2, ensure_ascii=False, default=str)
        )
    ...
    def _safe(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            return {
                k: "[REDACTED]" if _SECRET_FIELD.search(str(k)) else self._safe(v)
